...
require '../../lib/ascii85_native.rb'

describe Ascii85Native do
  ...
  it "#decode should be the inverse of #encode" do
    ...
    encoded = Ascii85Native.encode(test_str)
    decoded = Ascii85Native.decode(encoded)
  ...
  describe "#encode" do
    it "should encode all specified test-cases correctly" do
      TEST_CASES.each_pair do |input, encoded|
        assert_equal Ascii85Native.encode(input, true), encoded
      end
    end
    ...
    it "should encode Strings in different encodings correctly" do
      ...
      assert_equal Ascii85Native.encode(input_EUC_JP, true), Ascii85Native.encode(input_binary, true)
    end

    # wrapping not implemented in Ascii85Native
...
#      # No wrap
#      #
#      #assert_equal Ascii85Native.encode(test_str, false).count("\n"), 0
#
#      #
#      # x characters per line, except for the last one
#      #
#      x = 2 + rand(255) # < test_str.length
#      encoded = Ascii85Native.encode(test_str, x)
#
...
#      assert_equal Ascii85Native.encode("\0" * 4, 4), "<~z\n~>"
#    end
  end
  ...
  describe "#decode" do
    it "should decode all specified test-cases correctly" do
      TEST_CASES.each_pair do |decoded, input|
        if String.new.respond_to?(:encoding)
          assert_equal Ascii85Native.decode(input), decoded.dup.force_encoding('ASCII-8BIT')
        else
          assert_equal Ascii85Native.decode(input), decoded
        end
      end
    end
    ...
    it "should accept valid input in encodings other than the default" do
      ...
      input_ascii85 = Ascii85Native.encode(input, true)
      ...
      Encoding.list.each do |encoding|
        ...
        begin
          ...
          assert_equal Ascii85Native.decode(to_test).force_encoding('UTF-8'), input
    ...
    it "should only process data within delimiters" do
      assert_empty Ascii85Native.decode("<~~>")
      assert_empty Ascii85Native.decode("Doesn't contain delimiters")
      assert_empty Ascii85Native.decode("Mismatched ~>   delimiters 1")
      assert_empty Ascii85Native.decode("Mismatched <~   delimiters 2")
      assert_empty Ascii85Native.decode("Mismatched ~><~ delimiters 3")

      assert_equal Ascii85Native.decode("<~;KZGo~><~z~>"),    "Ruby"
      assert_equal Ascii85Native.decode("FooBar<~z~>BazQux"), "\0\0\0\0"
    end

    it "should ignore whitespace" do
      decoded = Ascii85Native.decode("<~6   #LdYA\r\08\n  \n\n- *rF*(i\"Ch[s \t(D.RU,@ <-\'jDJ=0\f/~>")
    ...
    it "should return ASCII-8BIT encoded strings" do
      ...
      assert_equal Ascii85Native.decode("<~;KZGo~>").encoding.name, "ASCII-8BIT"
    end
